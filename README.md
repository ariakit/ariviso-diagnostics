...
2. Pin `.github/workflows/visonaut-diagnostic.yml` to the reviewed reusable workflow commit while its source pull request is open, and keep that source branch available. This permits the signed diagnostic check to validate the pull request. After the source pull request merges, update the caller and service together to its full merge commit SHA before the Ariakit cutover. The workflow does not support manual image capture. `visonaut-probe.yml` remains available for a manual, no-OIDC environment probe.
